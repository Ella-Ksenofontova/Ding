...
function TextareaWithFormattingButtons({ id, text, setText }: TextareaProps) {
    ...
    function checkTarget(target: HTMLElement | null) {
        if (target) {
            if (target.classList.contains("url-input-field") || target.classList.contains("formatting-button") || target.classList.contains("url-input-button") || target.classList.contains("textarea-with-text") || target.classList.contains("link-like")) return;
    ...
    return (
        <div className="text-edit-wrapper">
            <Form.Control as="textarea" id={id} value={text} onChange={(event) => setText(event.target.value)}
                onSelect={event => {
                    const area = event.target as HTMLTextAreaElement;
                    setSelectedPositions({ start: area.selectionStart, end: area.selectionEnd });
                }} className="textarea-with-text" onBlur={handleBlur} />
            <div className="formatting-buttons-panel">
                ...
                <div className="link-wrapper">
                    ...
                    <div className="url-input" hidden={!isUrlFieldOpen}>
                        ...
                        <Form.Control className="url-input-field" id="url" value={url} onChange={event => setUrl(event.target.value)} autoComplete="off" />
